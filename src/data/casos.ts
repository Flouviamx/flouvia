...
export const casos: CaseStudy[] = [
...

  {
    ...
    solutionPoints: [
      { es: 'Storefront headless con Next.js 15 (App Router) + Shopify Storefront API para catálogo, carrito y checkout.', en: 'Headless storefront with Next.js 15 (App Router) + Shopify Storefront API for catalog, cart and checkout.' },
      { es: 'Motor de recomendaciones propio: segmenta por género y tipo de prenda en cada superficie del sitio (producto, "Queda bien con...", carrito) — no el algoritmo genérico de Shopify sin filtrar.', en: 'Custom recommendation engine: segments by gender and garment type across every surface (product page, "Complete the look", cart) — not Shopify\'s unfiltered generic algorithm.' },
      { es: 'Catálogo con fotografía real de producto matcheada por color en cada línea, con caída a un placeholder de marca donde aún falta foto.', en: 'Catalog with real product photography matched by color across every line, falling back to a branded placeholder where a photo is still missing.' },
      { es: 'Revalidación automática vía webhooks de Shopify: el catálogo se actualiza solo al cambiar un producto, sin rebuild manual.', en: 'Automatic revalidation via Shopify webhooks: the catalog updates itself the moment a product changes, no manual rebuild.' },
      { es: 'Contacto y newsletter con envío real vía Resend — sin apps de terceros ni Shopify Email.', en: 'Contact form and newsletter with real delivery via Resend — no third-party apps, no Shopify Email.' },
      { es: 'Navbar con mega menu animado, buscador que hace morph in-place (GSAP Flip) y carrito personalizado con upsell de un clic.', en: 'Navbar with animated mega menu, an in-place morphing search bar (GSAP Flip), and a personalized cart with one-click upsell.' },
